...
class LSTMRNN(object):
    def __init__(self, n_steps, input_size, output_size, cell_size, batch_size):
        ...
        with tf.name_scope('train'):
            ...
            #self.train_op =  tf.train.Optimizer(LR).minimize(self.cost)
    ...
    def add_output_layer(self):
        with tf.name_scope("add_output_layer"):
            ...
            with tf.name_scope('Wx_plus_b'):
                ...
                tf.summary.histogram(name = '/Outputs', values=self.pred)
                # self.train_op =  tf.train.Optimizer(LR).minimize(self.cost)
    ...
    def compute_accuracy(self):
        with tf.name_scope('average_accuracy'):
            correct_pred = tf.equal(tf.argmax(tf.nn.softmax(self.pred), 1), tf.argmax(self.ys, 1))
    ...
    def _weight_variable(self, shape, name='weights'):
        with tf.name_scope("Input_weight_variable"):
            initializer = tf.random_normal_initializer(mean=0., stddev=1.,)
            #tf.summary.histogram(name = '/Weights',values=self._weight_variable(shape,name))
            return tf.get_variable(shape=shape, initializer=initializer, name=name)

    def _bias_variable(self, shape, name='biases'):
        with tf.name_scope("Input_bias_variable"):
            initializer = tf.constant_initializer(0.1)
            #tf.summary.histogram(name = '/Biases',values=self._bias_variable(shape,name))
