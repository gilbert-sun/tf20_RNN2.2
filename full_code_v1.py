...
label_reshape_all0 = np.array([])

Vdata_reshape_all0 = np.array([])

Tdata_reshape_all0 = np.array([])

Sdata_reshape_all0 = np.array([])
...
def read_batch(isTrain):
    if(isTrain):
        return get_batch_file("./OutCsv/amita/*.csv")
    else:
        return get_batch_file("./TestCsv/amita/*.csv")


def get_batch_file(folderName):
    ...
    global Vdata_reshape_all

    global Tdata_reshape_all

    global Sdata_reshape_all

    global label_reshape_all
...
#                    label_reshape_all.append(4.0)
        ...
        print ("------------*******Total_data_lens*********--------------------------"+str( Total_data_lens ))

        fp.close()

    return Total_data_lens , label_reshape_all, Vdata_reshape_all,Sdata_reshape_all,Tdata_reshape_all



def get_batch(Total_data_lens , label_reshape_all, Vdata_reshape_all,Sdata_reshape_all,Tdata_reshape_all,EPOCH_No):
    ...
    # N0 = (np.shape(label_reshape_all)/4/ BASE_SIZE_DIV )
    N0 = (int(Total_data_lens / BASE_SIZE_DIV) )# - 3)
    N1= 50

    RADON_SHIFT = EPOCH_No % N0
    # RADON_SHIFT = EPOCH_No % 38
    print("%%%%%%%%%%%%%%%%%%%%%--------------N0:" + str(N0) + ":EPOCH:" + str(EPOCH_No)+ ":RADON_SHIFT:" + str(RADON_SHIFT))
...
#==========================================// Total_data_lens , label_reshape_all, [V,S,T]data_reshape_all
...
if __name__ == '__main__':
    IS_TRAINING = True
    Total_data_lens0, label_reshape_all0, Vdata_reshape_all0, Sdata_reshape_all0, Tdata_reshape_all0 =  read_batch(True)
    seq1, res1, xs1 , full_length1  = get_batch(Total_data_lens0, label_reshape_all0, Vdata_reshape_all0, Sdata_reshape_all0, Tdata_reshape_all0, 0)#read_batch(True)#get_batch("./OutCsv/amita/*.csv" , 0)  #read_batch(True)#
    # get_batch(read_batch(True), 0)
    print(" \n\n0 -------------------------------------------------------------------")
    print("Total_data_lens0:" + str( Total_data_lens0 ))
    ...
    model = LSTMRNN(TIME_STEPS, INPUT_SIZE, OUTPUT_SIZE, CELL_SIZE , full_length1 , RNN_LAYER_SIZE , DROP_OUT_RATE, True )
    sess = tf.Session()
    ...
    for i in range(2001):
        ...
        seq, res, xs , full_length =  get_batch(Total_data_lens0, label_reshape_all0, Vdata_reshape_all0, Sdata_reshape_all0, Tdata_reshape_all0, i) #full_length1 #
        print ("*******------gggg-----------------"+ str(full_length))
